...
export class tagController {
  constructor(private tag: tagServices) {}

    async tagEstoqueId(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const contratoLocalizado = await this.tag.obterTagsEstoqueContrato(contratoId);
    ...
    async tagAcoes(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const resultado = await this.tag.acoes(dadosDoContrato);
