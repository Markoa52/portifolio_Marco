...
import excluirContratoUsuarioQueue from '../queues/excluiContratoUsuarioQueue.js';
import { UsuarioService } from '../services/usuarioService.js';

// DEFINIÇÃO DOS NOMES DA DLX (Padrão de mercado baseado na sua fila atual)
const DLX_EXCHANGE_NAME = `${excluirContratoUsuarioQueue.nome}.dlx`;
const DLQ_QUEUE_NAME = `${excluirContratoUsuarioQueue.nome}.dlq`;
const DLQ_ROUTING_KEY = `${excluirContratoUsuarioQueue.nome}.failed`;
...
export async function iniciarConsumer(): Promise<void> {
    try {
        ...
        const channel = await connectRabbit(excluirContratoUsuarioQueue.nome);
        ...
        await channel.assertQueue(excluirContratoUsuarioQueue.nome, { 
            durable: true,
            arguments: {
        ...
        await channel.bindQueue(
            excluirContratoUsuarioQueue.nome, 
            'reports.exchange',                // Mesma Exchange usada no Agendador
            'reports.v1.trigger.exclui-ContratoUsuario'  // Mesma Routing Key usada no Agendador
        );

        console.log(`Aguardando mensagens na fila: ${excluirContratoUsuarioQueue.nome}`);
        console.log(`Proteção Dead Letter ativa. Falhas irão para: ${DLQ_QUEUE_NAME}`);


        channel.prefetch(1);

        channel.consume(excluirContratoUsuarioQueue.nome, async (msg: amqp.Message | null) => {
        if (!msg) return;
