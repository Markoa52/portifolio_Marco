...
import excluirUsuarioQueue from '../queues/excluiUsuarioQueue.js';
import { UsuarioService } from '../services/usuarioService.js';

// DEFINIÇÃO DOS NOMES DA DLX (Padrão de mercado baseado na sua fila atual)
const DLX_EXCHANGE_NAME = `${excluirUsuarioQueue.nome}.dlx`;
const DLQ_QUEUE_NAME = `${excluirUsuarioQueue.nome}.dlq`;
const DLQ_ROUTING_KEY = `${excluirUsuarioQueue.nome}.failed`;
...
export async function iniciarConsumer(): Promise<void> {
    try {
        ...
        const channel = await connectRabbit(excluirUsuarioQueue.nome);
        ...
        await channel.assertQueue(excluirUsuarioQueue.nome, { 
            durable: true,
            arguments: {
        ...
        await channel.bindQueue(
            excluirUsuarioQueue.nome, 
            'reports.exchange',                // Mesma Exchange usada no Agendador
            'reports.v1.trigger.exclui-usuario'  // Mesma Routing Key usada no Agendador
        );

        console.log(`Aguardando mensagens na fila: ${excluirUsuarioQueue.nome}`);
        console.log(`Proteção Dead Letter ativa. Falhas irão para: ${DLQ_QUEUE_NAME}`);


        channel.prefetch(1);

        channel.consume(excluirUsuarioQueue.nome, async (msg: amqp.Message | null) => {
        if (!msg) return;
