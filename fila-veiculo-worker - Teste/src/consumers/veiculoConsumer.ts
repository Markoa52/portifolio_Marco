...
import veiculoQueue from '../queues/veiculoQueue.js';
import { CriaVeiculoService } from '../services/criaVeiculoService.js';

// DEFINIÇÃO DOS NOMES DA DLX (Padrão de mercado baseado na sua fila atual)
const DLX_EXCHANGE_NAME = `${veiculoQueue.nome}.dlx`;
const DLQ_QUEUE_NAME = `${veiculoQueue.nome}.dlq`;
const DLQ_ROUTING_KEY = `${veiculoQueue.nome}.failed`;
...
export async function iniciarConsumer(): Promise<void> {
    try {
        ...
        const channel = await connectRabbit(veiculoQueue.nome);
        ...
        await channel.assertQueue(veiculoQueue.nome, { 
            durable: true,
            arguments: {
        ...
        await channel.bindQueue(
            ...
            'reports.v1.trigger.criar-veiculo'  // Mesma Routing Key usada no Agendador
        );

        console.log(`Aguardando mensagens na fila: ${veiculoQueue.nome}`);
        console.log(`Proteção Dead Letter ativa. Falhas irão para: ${DLQ_QUEUE_NAME}`);


        channel.prefetch(1);

     channel.consume(veiculoQueue.nome, async (msg: amqp.Message | null) => {
    if (!msg) return;
    ...
    try {
        ...
        if (!payload.js.metadata || !payload.js.contextoVeiculo) {
        ...
        await CriaVeiculoService.processarCadastroRelacional(payloadDecodificado);
