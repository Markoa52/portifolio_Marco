...
import faturaQueue from '../queues/faturamentoQueue.js';
import { faturamento } from '../services/faturamentoServices.js';

// DEFINIÇÃO DOS NOMES DA DLX (Padrão de mercado baseado na sua fila atual)
const DLX_EXCHANGE_NAME = `${faturaQueue.nome}.dlx`;
const DLQ_QUEUE_NAME = `${faturaQueue.nome}.dlq`;
const DLQ_ROUTING_KEY = `${faturaQueue.nome}.failed`;

export async function iniciarConsumer(): Promise<void> {
    try {
        // Coleta o canal retornado diretamente
        const channel = await connectRabbit(faturaQueue.nome);
        ...
        await channel.assertQueue(faturaQueue.nome, { 
            durable: true,
            arguments: {
        ...
        await channel.bindQueue(
            ...
            'reports.v1.trigger.fila-faturamento-cliente'  // Mesma Routing Key usada no Agendador
        );

        console.log(`Aguardando mensagens na fila: ${faturaQueue.nome}`);
        console.log(`Proteção Dead Letter ativa. Falhas irão para: ${DLQ_QUEUE_NAME}`);

        channel.prefetch(1);

        channel.consume(faturaQueue.nome, async (msg) => {
            if (!msg) return;

            console.log(`[Excel Worker] Nova mensagem detectada na fila ${faturaQueue.nome}!`);

            try {
                const dados = JSON.parse(msg.content.toString());
                
                // Executa a lógica que gera a planilha Excel
                await faturamento.processarCadastroRelacional(dados);
