...
class ContratoRepository {
  ...
  async criarContrato(contratoData: any, personId: any, transaction: any) {
    ...
    request.input('documentNumber', sql.VarChar, contratoData.documentNUmber);
    ...
    const query = `
      ...
      VALUES (@dataInicio, @planoComercializado, @valorMensalidade, @valorTag, @planoPagamento, @dataCorteFaturamento, @diaFaturamento, @documentNumber);
    `;
  ...
  async criarPerson(personData: any, transaction: any) {
    ...
    request.input('contractId', sql.VarChar, personData.contractId);

    const query = `
      INSERT INTO person (cnpj, nome_empresa, contractId) VALUES (@cnpj, @nomeEmpresa, (select id from contract where @cnpj=documentNumber));
    ...
    async criarEndereco(enderecoData: any, transaction: any) {
    const request = new sql.Request(transaction);
    ...
    request.input('personId', sql.int, enderecoData.PersonId);
    request.input('documentNumber', sql.VarChar, enderecoData.documentNUmber);

    const query = `
      INSERT INTO endereco (cep, rua, numero, complemento, bairro, cidade, estado, person_id)
      VALUES (@cep, @rua, @numero, @complemento, @bairro, @cidade, @estado, (select personId from person where cnpj = @documentNumber), @documentNumber );
    `;
    await request.query(query);
  }
