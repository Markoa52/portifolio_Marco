...
import pedidoQueue from '../queues/pedidoQueue.js';
import { cadastroPedidoService } from '../services/pedidoServices.js';

// DEFINIÇÃO DOS NOMES DA DLX (Padrão de mercado baseado na sua fila atual)
const DLX_EXCHANGE_NAME = `${pedidoQueue.nome}.dlx`;
const DLQ_QUEUE_NAME = `${pedidoQueue.nome}.dlq`;
const DLQ_ROUTING_KEY = `${pedidoQueue.nome}.failed`;
...
export async function iniciarConsumer(): Promise<void> {
    try {
        ...
        const channel = await connectRabbit(pedidoQueue.nome);
        ...
        await channel.assertQueue(pedidoQueue.nome, { 
            durable: true,
            arguments: {
        ...
        await channel.bindQueue(
            ...
            'reports.v1.trigger.criar-pedido'  // Mesma Routing Key usada no Agendador
        );

        console.log(`Aguardando mensagens na fila: ${pedidoQueue.nome}`);
        console.log(`Proteção Dead Letter ativa. Falhas irão para: ${DLQ_QUEUE_NAME}`);


        channel.prefetch(1);

     channel.consume(pedidoQueue.nome, async (msg: amqp.Message | null) => {
    if (!msg) return;
    ...
    try {
        ...
        if (!payload.js.metadata || !payload.js.contextoPedido) {
