...
export const Contrato: React.FC<IContratoProps> = ({ setPaginaAtiva, paginaAtiva }) => {
  ...
  <header className="navbar navbar-light bg-white py-3 px-3 shadow-sm w-100" style={{ maxWidth: "1200px", margin: "0 auto", borderRadius: "0 0 12px 12px", borderBottom: "1px solid #e2e8f0" }}>
      <div className="container d-flex justify-content-between align-items-center">
        
        {/* ==========================================
            1. SEÇÃO DE MENUS E NAVEGAÇÃO (ESQUERDA)
            ========================================== */}
        <div className="d-flex align-items-center gap-2">
          
          {/* MENU HAMBÚRGUER MOBILE */}
          <div className="d-block d-md-none me-2 text-dark">
            <MenuMobileModulos setAbaAtiva={setAbaAtiva} />
          </div>

                 {/* ==========================================
            2. MENU HAMBÚRGUER DESKTOP
            ========================================== */}
        <div className="d-none d-md-block mx-3">
          ...
          {/* ITEM 1: INÍCIO */}
          <div 
            className="btn btn-menu-limpo d-flex flex-column align-items-center p-3 text-decoration-none text-dark border-0" 
            onClick={() => { setAbaAtiva('cards-gerais'); setMenuAberto(null); }}
            style={{ minWidth: '100px', transition: 'background-color 0.2s' }}
          >
            {/* MUDANÇA: Substituído img por <Home /> com cor grafite (#475569) */}
          ...
          <div className="dropdown">
            <button 
              className={`btn btn-menu-limpo d-flex flex-column align-items-center p-3 dropdown-toggle fs-5 fw-bold text-dark border-0 ${menuAberto === 'frota' ? 'show' : ''}`}
              onClick={() => alternarSubmenu('frota')}
              style={{ minWidth: '100px', transition: 'background-color 0.2s' }}
            >
              {/* MUDANÇA: Substituído img por <Truck /> */}
          ...
          <div className="dropdown">
            <button 
              className={`btn btn-menu-limpo d-flex flex-column align-items-center p-3 dropdown-toggle fs-5 fw-bold text-dark border-0 ${menuAberto === 'faturas' ? 'show' : ''}`}
              onClick={() => alternarSubmenu('faturas')}
              style={{ minWidth: '100px', transition: 'background-color 0.2s' }}
            >
              {/* MUDANÇA: Substituído img por <FileText /> */}
          ...
          <div className="dropdown">
            <button 
              className={`btn btn-menu-limpo d-flex flex-column align-items-center p-3 dropdown-toggle fs-5 fw-bold text-dark border-0 ${menuAberto === 'relatorios' ? 'show' : ''}`}
              onClick={() => alternarSubmenu('relatorios')}
              style={{ minWidth: '100px', transition: 'background-color 0.2s' }}
            >
              {/* MUDANÇA: Substituído img por <BarChart3 /> */}
        ...
        {/* ==========================================
            3. BLOCOS DE INFORMAÇÕES FINANCEIRAS (DIREITA)
            ========================================== */}
        <div className="d-flex align-items-center gap-5 text-dark text-end">
          ...
          <div className="d-flex flex-column align-items-start me-3">
            ...
            <div className="d-flex align-items-center gap-1 mt-1">
              ...
              <div onClick={() => setAbaAtiva('detalhes-pedagio')} className="cp d-flex align-items-center">
                {/* MUDANÇA: Substituído img de detalhes por <Info /> */}
          ...
          {/* GASTOS ATUAIS */}
          <div className="d-flex flex-column align-items-start" style={{ width: '180px' }}>
            <div className="d-flex justify-content-between w-100 align-items-center">
              <span className="text-muted fw-bold" style={{ fontSize: '0.65rem', letterSpacing: '0.05em' }}>GASTOS ATUAIS</span>
              <span className="fw-bold text-primary" style={{ fontSize: '0.65rem' }}>
                {Math.round(porcentagemConsumida)}%
              </span>
            </div>
          
            <span className="fs-5 fw-bold mt-1 text-dark">{formatarMoeda(valorGasto)}</span>
          
            <div className="progress w-100 bg-light mt-1" style={{ height: '6px', border: '1px solid #e2e8f0' }}>
              <div 
                className={`progress-bar ${porcentagemConsumida >= 90 ? 'bg-danger' : 'bg-primary'}`}
                role="progressbar"
                style={{ width: `${porcentagemConsumida}%` }}
                aria-valuenow={porcentagemConsumida}
              ></div>
            </div>
            <span className="text-muted" style={{ fontSize: '0.6rem', marginTop: '2px' }}>
  ...
  <main className="container my-4 p-0" style={{ maxWidth: "1200px", margin: "0 auto" }}>
      ...
      <div className="col-md-6 ps-0">
        <div className="card p-4 h-100 shadow-sm border border-light-subtle bg-white">
          <div className="d-flex justify-content-between align-items-center mb-3">
            ...
            <button className="btn btn-dark btn-sm fw-semibold" style={{ fontSize: '0.8rem' }} onClick={() => setAbaAtiva('historico-fatura')}>
              Consultar faturas →
            </button>
          </div>
...
{/* CARD 2: Últimos Pedidos (Compactado para ficar do mesmo tamanho do Card 1) */}
<div className="col-md-6 pe-0">
  {/* MUDANÇA: p-3 deixa o card ligeiramente mais compacto para caber na mesma altura */}
  <div className="card p-3 h-100 shadow-sm border border-light-subtle bg-white d-flex flex-column justify-content-between">
    
    {/* Cabeçalho do Card */}
    {/* MUDANÇA: Reduzido o mb-4 para mb-2 para ganhar espaço vertical */}
    <div className="d-flex justify-content-between align-items-center mb-2">
      <h3 className="fs-6 mb-0 fw-bold text-dark">Últimos pedidos</h3>
      <button className="btn btn-dark btn-sm fw-semibold" style={{ fontSize: '0.8rem', padding: '0.2rem 0.5rem' }} onClick={() => setAbaAtiva('historico-fatura')}>
        Consultar pedidos →
      </button>
    </div>

    {/* CONTAINER GERAL DA TIMELINE */}
    {/* MUDANÇA: pt-1 e mt-1 reduzem as margens superiores da linha do tempo */}
    <div className="position-relative px-2 mt-1 pt-1">
      
      {/* 1. LINHA CONECTORA CINZA DE FUNDO */}
      <div 
        className="position-absolute start-0 end-0 bg-secondary-subtle" 
        style={{ height: '2px', zIndex: 0, top: '10px' }}
      ></div>
      
      {/* 2. FILA EXCLUSIVA DAS BOLINHAS */}
      {/* MUDANÇA: mb-1 reduz o espaço entre as bolinhas e os textos */}
      <div className="d-flex justify-content-between text-center position-relative mb-1" style={{ zIndex: 1 }}>
        ...
        <div style={{ width: '30%' }}>
          <span className="rounded-circle bg-primary text-white d-inline-flex align-items-center justify-content-center fw-bold lh-1" style={{ width: '20px', height: '20px', fontSize: '0.7rem' }}>2</span>
        </div>
        <div style={{ width: '30%' }}>
          <span className="rounded-circle bg-secondary-subtle text-secondary border d-inline-flex align-items-center justify-content-center fw-bold lh-1" style={{ width: '20px', height: '20px', fontSize: '0.7rem' }}>3</span>
        </div>
      </div>

      {/* 3. FILA EXCLUSIVA DOS TEXTOS (Mais compactos e grudados) */}
      <div className="d-flex justify-content-between text-center">
        ...
        {/* Etapa 3 */}
        <div style={{ width: '30%' }}>
          {/* MUDANÇA: Reduzido levemente o tamanho do texto para 0.75rem para não criar uma caixa gigante no wrap */}
