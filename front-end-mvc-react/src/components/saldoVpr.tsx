...
// 1. CORREÇÃO: Inicializa com um array vazio [] para o seu .map() não quebrar na primeira renderização
 ...
 return (
  ...
  <div className="container-fluid p-0 text-start">
    ...
    <div className="pb-3 mb-0 px-2 px-sm-0">
      <h2 className="fs-4 fw-bold text-dark m-0">Detalhamento do Vale Pedágio</h2>
    </div>

    {/* ====================================================================
        📊 CONTROLE DE PLACAS E SALDOS INTEGRADO
        ==================================================================== */}
    <div className="card p-3 p-md-4 shadow-sm border border-light-subtle bg-white rounded-3 w-100">
      
      {/* Sub-cabeçalho da Tabela - Ajustado flex para empilhar no telemóvel se necessário */}
      <div className="p-3 bg-light border-bottom d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-2">
        <div>
          <div className="d-flex align-items-center gap-2">
            <History size={24} />
            <h5 className="fs-6 fw-bold text-dark m-0">🚗 Saldo de Veículos e Frotas</h5>
          </div>
          <small className="text-muted d-block mt-1">Acompanhamento de saldos ativos para passagens em pedágios.</small>
        </div>
        <span className="badge bg-success-subtle text-success border border-success-subtle px-2.5 py-1.5 fw-bold align-self-start align-self-sm-center">
          {saldoVeiculoVPR?.length || 0} Cadastrados
        </span>
      </div>

      {/* 🖥️ VISTA PARA DESKTOP (Oculta em telemóveis: d-none d-md-block) */}
      ...
      {/* 📱 VISTA PARA TELEMÓVEL (Oculta em computadores: d-block d-md-none) */}
      <div className="d-block d-md-none">
        {saldoVeiculoVPR && saldoVeiculoVPR.length > 0 ? (
          saldoVeiculoVPR.map((veiculo: any, index: number) => (
            <div 
              key={`card-${veiculo.placa || index}-${index}`} 
              className={`p-3 ${index !== saldoVeiculoVPR.length - 1 ? 'border-bottom' : ''}`}
            >
              ...
              {/* Linha do meio: Modelo e Detalhes */}
              <div className="mb-2">
                <div className="fw-semibold text-dark" style={{ fontSize: '0.9rem' }}>{veiculo.modelo || "Não informado"}</div>
                <small className="text-muted text-uppercase" style={{ fontSize: '0.7rem' }}>
                  Eixos: {veiculo.eixo || "1"} | Tipo: {veiculo.tipoveiculo === '2' ? 'Pesado' : 'Passeio'}
                </small>
              </div>
              ...
              <div className="d-flex justify-content-between align-items-center bg-light p-2 rounded">
                <span className="text-muted small">Saldo da Conta:</span>
